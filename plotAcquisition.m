...
t = ceil(nrFound/s);
codeDelay = 0:1:1023;
dopplerFreq = -6000:100:6000;

[Y, X] = meshgrid(dopplerFreq, codeDelay);
...
%Muneeb Editing Start for 3D Mesh
% Plot spectrum plots in one figure
figure('Position',[200,50,1000,600])
ind = 1;
for i=1:acqResults.nrObs
    if(acqResults.channel(i).bFound == true)    
        ...
        AcquisitionValueVector = acqResults.channel(i).spec;
        PRN = acqResults.channel(i).SvId.satId;
        subplot(s,t,ind);

        requiredSize = 1024*121;
        PaddedAcquisitionValueVector = [AcquisitionValueVector, NaN(1, requiredSize - length(AcquisitionValueVector))];;
        AcquisitionMatrix = reshape(PaddedAcquisitionValueVector, 1024, 121);

        mesh(X, Y, AcquisitionMatrix);
        title('GNSS Acquisition Correlation 3D Mesh Plot');
...
%Muneeb Editing End for 3D Mesh
